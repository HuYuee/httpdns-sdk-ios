...
// 高级功能标记：bit0 表示仅使用 HttpDNS（HttpOnly）
#define MSDKDNS_FEATURE_HTTP_ONLY (1 << 0)
// 高级功能标记：bit1 表示开启域名保活能力（keepAliveDomains 列表非空）
#define MSDKDNS_FEATURE_KEEP_ALIVE_DOMAINS (1 << 1)
// 高级功能标记：bit2 表示开启使用过期 IP 能力（乐观 DNS）
#define MSDKDNS_FEATURE_EXPIRED_IP_ENABLED (1 << 2)
// 高级功能标记：bit3 表示开启持久化缓存能力
#define MSDKDNS_FEATURE_PERSIST_CACHE_IP (1 << 3)
// 高级功能标记：bit4 表示开启预解析能力（preResolvedDomains 列表非空）
#define MSDKDNS_FEATURE_PRE_RESOLVE_DOMAINS (1 << 4)
